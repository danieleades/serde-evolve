...
fn generate_rep_enum(rep_name: &syn::Ident, version_types: &[syn::Path]) -> TokenStream {
    ...
    let variants = version_types.iter().enumerate().map(|(idx, ty)| {
        ...
        let variant_doc = format!("Version {} of the serialized format.", idx + 1);
        quote! {
            #[doc = #variant_doc]
    ...
    let enum_doc = format!("Versioned representation enum for serialization.\n\nThis enum is automatically generated by the `Versioned` derive macro and contains variants for each version in the migration chain.");
